...
namespace hr::renderPlatform
{
    ...
    public:
        ...
        bool rawInputGetKeyStatus(unsigned int vcode);
        bool rawInputGetKeyStatus(Window::VirtualKeys vcode);
        ...
        if ((messageID == WM_CHAR) || (messageID == WM_KEYDOWN) || (messageID == WM_MOUSEWHEEL))
        {
            ...
            if ((messageID == WM_CHAR) || (messageID == WM_KEYDOWN))
            {
                ...
                if (messageID == WM_CHAR)
                    ...
                    window->mEvents.queue[window->mEvents.queueSize++] =
                      Window::Message(Window::Message::MessageType::VirtualKey, WindowImpl::translateVirtualKeyCode(wParam), flags);
            }
    ...
    int32_t WindowImpl::translateVirtualKeyCode(LPARAM nativeKeyCode)
    {
        ...
        {
            case VK_LBUTTON:
                virtuakKey = Window::VirtualKeys::MouseLButton;
                break;
            case VK_RBUTTON:
                virtuakKey = Window::VirtualKeys::MouseRButton;
                break;
            case VK_MBUTTON:
                virtuakKey = Window::VirtualKeys::MouseMButton;
                break;
            case VK_TAB:
                virtuakKey = Window::VirtualKeys::Tab;
                break;
            case VK_BACK:
                virtuakKey = Window::VirtualKeys::Backspace;
                break;
            case VK_RETURN:
                virtuakKey = Window::VirtualKeys::Return;
                break;
            case VK_SHIFT:
                virtuakKey = Window::VirtualKeys::Shift;
                break;
            case VK_CONTROL:
                virtuakKey = Window::VirtualKeys::Control;
                break;
            case VK_ESCAPE:
                virtuakKey = Window::VirtualKeys::Escape;
                break;
            case VK_SPACE:
                virtuakKey = Window::VirtualKeys::Space;
                break;
            case VK_PRIOR:
                virtuakKey = Window::VirtualKeys::PageUp;
                break;
            case VK_NEXT:
                virtuakKey = Window::VirtualKeys::PageDown;
                break;
            case VK_END:
                virtuakKey = Window::VirtualKeys::End;
                break;
            case VK_HOME:
                virtuakKey = Window::VirtualKeys::Home;
                break;
            case VK_LEFT:
                virtuakKey = Window::VirtualKeys::Left;
                break;
            case VK_RIGHT:
                virtuakKey = Window::VirtualKeys::Right;
                break;
            case VK_UP:
                virtuakKey = Window::VirtualKeys::Up;
                break;
            case VK_DOWN:
                virtuakKey = Window::VirtualKeys::Down;
                break;
            case VK_DELETE:
                virtuakKey = Window::VirtualKeys::Delete;
                break;
            default:
                virtuakKey = Window::VirtualKeys::Invalid;
                break;
        }
    ...
    bool WindowImpl::windowInit(std::string_view windowTitle,
                                Window::WindowStyle style,
                                bool targetSecondaryDisplay,
                                const unsigned int targetWidth,
                                const unsigned int targeHeight)
    {
        if (mIsInitialized)
        {
        ...
        if (style == Window::WindowStyle::StyleFullscreen)
        {
            ...
            mHWnd = CreateWindowEx(dwExStyle, mClassName.c_str(), windowTitleWChar.c_str(), dwStyle, 0, 0, mOriginalDeviceMode.dmPelsWidth, mOriginalDeviceMode.dmPelsHeight,
                                   HWND_DESKTOP, nullptr, mHModule, this);
        ...
        else if (style == Window::WindowStyle::StyleFullscreenWindow)
        {
            ...
            mHWnd = CreateWindowEx(dwExStyle, mClassName.c_str(), windowTitleWChar.c_str(), dwStyle, monitorRect.left, monitorRect.top, monitorRect.right - monitorRect.left,
                                   monitorRect.bottom - monitorRect.top, HWND_DESKTOP, nullptr, mHModule, this);
        ...
        else if (style == Window::WindowStyle::StyleWindow)
        {
            ...
            mHWnd = CreateWindowEx(dwExStyle, mClassName.c_str(), windowTitleWChar.c_str(), dwStyle, monitorRect.left, monitorRect.top, (windowRect.right - windowRect.left),
                                   (windowRect.bottom - windowRect.top), HWND_DESKTOP, nullptr, mHModule, this);
    ...
    {
        std::lock_guard<std::mutex> _(mRawInput.lock);
    ...
    bool WindowImpl::rawInputGetKeyStatus(unsigned int vcode)
    {
        std::lock_guard<std::mutex> _(mRawInput.lock);
    ...
    bool WindowImpl::rawInputGetKeyStatus(Window::VirtualKeys vcode)
    {
    ...
    /**
     * OpenglContextImpl implementation
     */
    ...
    OpenglContextImpl::OpenglContextImpl(const WindowImpl& window,
                                         std::string_view openGLModuleName,
                                         int contextMajorVersion,
                                         int contextMinorVersion,
                                         bool contextDebug,
      ...
      : mWindow{window}
    {
...

        {
            ...
            int iAttributes[] = {WGL_PIXEL_TYPE_ARB,
                                 WGL_TYPE_RGBA_ARB,
        ...
        {
            int contextAttrib[] = {WGL_CONTEXT_MAJOR_VERSION_ARB,
                                   contextMajorVersion,
                                   WGL_CONTEXT_MINOR_VERSION_ARB,
                                   contextMinorVersion,
                                   WGL_CONTEXT_FLAGS_ARB,
                                   (contextDebug ? WGL_CONTEXT_DEBUG_BIT_ARB : 0) | (contextForwardCompatible ? WGL_CONTEXT_FORWARD_COMPATIBLE_BIT_ARB : 0),
    ...
    Window::Window(hr::engine::Logger& logger)
      : mImpl{std::make_unique<WindowImpl>(logger)}
    {}

    Window::~Window() noexcept
    {}

    std::string_view Window::getErrorMsg() const
    {
    ...
    std::string_view OpenglContext::getErrorMsg() const
    {
        return mImpl->getErrorMsg();
    }
    ...
    bool OpenglContext::swapBuffers(void) const
    {
