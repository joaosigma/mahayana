export module gal:tools.viewport.impl;
...
namespace hr::gl::tools
{
    ...
    void Viewport::projectPoint(ProjectionType projType, const hr::Matrix4f& modelView, std::span<hr::Vector3f> points) const
    {
        if (points.empty())
            return;
        ...
        for (auto& p : points)
        {
            auto result = p.convert<float, 4>(1.0f);
