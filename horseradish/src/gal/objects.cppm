...
export namespace hr::gl::objects
{
    class ObjectGL
    {
    protected:
    ...
    public:
        ...
        enum class StorageType
        {
            ...
            COMPRESSED_BC1,
            COMPRESSED_SRGB_BC1, // RGB
            COMPRESSED_BC3,
            COMPRESSED_SRGB_BC3, // RGBA
            COMPRESSED_BC4,      // Mono (grayscale)
            COMPRESSED_BC5,      // Dual (2xgrayscale)
            COMPRESSED_BC7,
            COMPRESSED_SRGB_BC7 // RGB and RGBA (best if A correlates to RGB)
        };
        enum class DataType
        {
            BYTE,
    ...
    public:
        ...
        bool
        uploadCubemapData(CubemapFace face, GLint level, GLint xOffset, GLint yOffset, GLint width, GLint height, DataFormat dataFormat, DataType dataType, const void* const data)
        {
    ...
    public:
        ...
        bool writeData(const void* const dataPtr, const size_t dataSize, const size_t bufferOffset) const
        {
            ...
            if (mMappedPtr)
            {
                std::memcpy(reinterpret_cast<unsigned char*>(mMappedPtr) + bufferOffset, dataPtr, dataSize);
            }
            else
            {
                auto mappedPtr = glMapNamedBufferRange(mId, bufferOffset, dataSize, GL_MAP_WRITE_BIT);
                std::memcpy(mappedPtr, dataPtr, dataSize);
                glUnmapNamedBuffer(mId);
            }
        ...
        bool readData(const size_t bufferOffset, const size_t dataSize, void* const dataPtr) const
        {
            ...
            if (mMappedPtr)
            {
                std::memcpy(dataPtr, reinterpret_cast<unsigned char*>(mMappedPtr) + bufferOffset, dataSize);
            }
            else
            {
                auto mappedPtr = glMapNamedBufferRange(mId, bufferOffset, dataSize, GL_MAP_READ_BIT);
                std::memcpy(dataPtr, mappedPtr, dataSize);
                glUnmapNamedBuffer(mId);
            }
