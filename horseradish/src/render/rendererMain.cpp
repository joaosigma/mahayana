...
namespace hr::render
{
    ...
    void RendererMain::passSky(Scene& scene, const tools::Camera& hrCamera, const hr::gl::tools::Viewport& hrViewport)
    {
        ...
        hr::gl::glProgramUniformMatrix4fv(mShaders.forwardPassSky.vertex.id(), mShaders.forwardPassSky.vertex.getUniformLocation("modelviewMatrix"), 1, false,
                                          matrixModelView.data().data());
        hr::gl::glProgramUniformMatrix4fv(mShaders.forwardPassSky.vertex.id(), mShaders.forwardPassSky.vertex.getUniformLocation("projectionMatrix"), 1, false,
                                          matrixProjection.data().data());
    ...
    void RendererMain::compositePostProcessing(const hr::gl::tools::Viewport& hrViewport)
    {
        ...
        hr::gl::glProgramUniformMatrix4fv(mShaders.postprocess.vertex.id(), mShaders.postprocess.vertex.getUniformLocation("projectionMatrix"), 1, false,
                                          hrViewport.getProjection(hr::gl::tools::Viewport::ProjectionType::Proj2D).data().data());
    ...
    void RendererMain::loadGeometry(Scene& scene, const IRenderManager& manager)
    {
        ...
        manager.iterateObjects(
          [&](IRenderObject& obj)
          {
              ...
              // vertices
              {
                  scene.mRenderData.vboMeshData.writeData(
                    [&obj](void* const destBuffer, size_t requestedDataSize)
                    {
              ...
              // indices
              {
                  scene.mRenderData.vboMeshIndexData.writeData(
                    [&obj](void* const destBuffer, size_t requestedDataSize)
                    {
...

        {
            ...
            scene.mRenderData.vboIndirectDraw.init(gl::objects::Buffer::Type::DrawIndirect, drawCommands.get(), sizeof(gl::objects::Buffer::DrawElementsIndirectCommand) * numGeoms,
                                                   gl::objects::Buffer::UsageType::ServerStatic);
        }
    ...
    void RendererMain::loadMaterialsTextures(Scene& scene, const IRenderManager& manager)
    {
        ...
        manager.iterateTextureSets(
          [this, &scene](IRenderTextureSet& textureSet)
          {
    ...
    RendererMain::RendererMain(const hr::gl::objects::Context& glContext, hr::io::FileSystem& fileSystem, size_t renderWidth, size_t renderHeight)
      : mFileSystem(fileSystem), mGlContext(glContext)
    {
        ...
        mFBOs.fboZPass.init();
        mFBOs.fboZPass.attachTDepth(mFBOs.texZ);
        mFBOs.fboZPass.isStatusComplete();

        mFBOs.fboForward.reset();
        mFBOs.fboForward.init();
        mFBOs.fboForward.attachTColor(mFBOs.texLighting, 0).attachTColor(mFBOs.texNormals, 1).attachTColor(mFBOs.texSpecular, 2).attachTDepth(mFBOs.texZ);
        ...
        // buffers
        mShaders.forwardPassBuffers.uniform.init(hr::gl::objects::Buffer::Type::UniformBuffer, sizeof(Shaders::UniformLayout),
                                                 hr::gl::objects::Buffer::UsageType::PersistentOnlyWrite);
        mShaders.forwardPassBuffers.storage.init(hr::gl::objects::Buffer::Type::ShaderStorage, sizeof(Shaders::LightLayout) * Shaders::LightLayoutMaxElements,
                                                 hr::gl::objects::Buffer::UsageType::PersistentOnlyWrite);
...

        {
            ...
            // shader storage buffer common to every pass is prepared/set here
            {
                ...
                else
                {
                    ...
                    auto vec = Vector3f::calcLerp(Vector3f{animLight.dir[0] - 10.0f, animLight.dir[1], animLight.dir[2]},
                                                  Vector3f{animLight.dir[0] + 10.0f, animLight.dir[1], animLight.dir[2]}, t);
