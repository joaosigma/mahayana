		...
		class World
		{
			...
			void buildBHV()
			{
				...
				for (const auto& objData : mObjects2)
				{
					...
					obj.iterateTris([&prims = meshData.prims](size_t triIndex, const geom::VertexFull& p1, const geom::VertexFull& p2, const geom::VertexFull& p3) -> bool
					{
						//Triangle<Vector3d> tri{ Vector3f{ p1.pos }.convert<double>(), Vector3f{ p2.pos }.convert<double>(), Vector3f{ p3.pos }.convert<double>() };

						bvh::Triangle<double> tri{
							bvh::Vector3<double>{p1.pos[0],p1.pos[1],p1.pos[2] },
							bvh::Vector3<double>{p2.pos[0],p2.pos[1],p2.pos[2] },
		...
		public:
			...
			bool hit(const Ray<Vector3d>& ray, double tMin, double tMax, Hit& hit) const
			{
...

				{
					...
					bvh::Ray<double> newRay{
						translate(ray.origin()), translate(ray.direction()),
						tMin, tMax
					};
				...
				//
				//	double rayT;
				//	auto wasHit = bvh.bvh.intersects(ray, tMin, tMax, [&obj = std::get<0>(targetObj), &meshHit](std::span<size_t> triIndices, const Ray<Vector3d>& ray, double rayDistMin, double rayDistMax, double& rayT)
				//	{				
				//		auto wasHit{ false };
