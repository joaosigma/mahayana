...
namespace hr::render
{
    ...
    static std::string_view PromptDefault = "console@main# "; // ASCII only
    ...
    void ConsoleUI::processMsgPrompt(const renderPlatform::Window::Message& msg)
    {
        ...
        else if (msg.getType() == renderPlatform::Window::Message::MessageType::CharacterKey)
        {
            ...
            if (keyVal == 22) // paste
            {
                ...
                if (!hr::platform::Platform::clipboardGetStrings(
                      [&](std::string curString) -> bool
                      {
                          strUTF8 = std::move(curString);
                          return false;
                      }))
                {
                    hr::platform::Platform::clipboardGetFiles(
                      [&](std::string curString) -> bool
                      {
                          strUTF8 = std::move(curString);
                          return false;
    ...
    void ConsoleUI::drawContent(size_t textSize, const hr::Matrix4f& transformMatrix) const
    {
        ...
        // draw prompt
        {
            ...
            guiFont->layout(unicodeStr, mTextRect.width,
                            [&](unsigned int curLine, unsigned int unicodeCharOffset, unsigned int unicodeCharCount)
                            {
        ...
        // draw logger history
        {
            ...
            mLogger.iterateBuffer(
              [&](const engine::Logger::EntryType entryType, const engine::Logger::ModuleType moduleType, const bool isMsgFormatted, std::string_view msg)
              {
            ...
            for (const auto& logMsg : logMsgs)
            {
                ...
                {
                    auto finalLineY = curLineY;
                    guiFont->layout(logMsg.msg, mTextRect.width,
                                    [&](unsigned int curLine, unsigned int unicodeCharOffset, unsigned int unicodeCharCount)
                                    {
                                        auto lineY = curLineY + static_cast<float>(curLine) * textHeight;
                                        finalLineY += textHeight;
    ...
    void ConsoleUI::drawBackground(const hr::Matrix4f& transformMatrix, float bkgAlpha) const
    {
        ...
        hr::gl::glProgramUniformMatrix4fv(mRenderer.mShaders.drawNoTex.progVertex.id(), mRenderer.mShaders.drawNoTex.progVertex.getUniformLocation("transformationMatrix"), 1,
                                          false, transformMatrix.data().data());
