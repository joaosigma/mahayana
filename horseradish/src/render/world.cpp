...
#pragma pack(pop)
        ...
        bool fileAddData(hr::streams::FileStream& fstream,
                         size_t numGeoms,
                         size_t numAnimSets,
                         size_t numAnims,
                         FileWriteDataCallback cbWriteData,
                         FileWriteDataCallback cbWriteGeomChunk,
                         FileWriteDataCallback cbWriteAnimSetChunk,
                         FileWriteDataCallback cbWriteAnimChunk)
        {
            ...
            if ((numGeoms > 0) || (numAnimSets > 0) || (numAnims > 0))
            {
                ...
                if (numGeoms > 0)
                {
                    geomChunksTmp = std::make_unique<GeomChunkInfo[]>(numGeoms);

                    freader.seek(hr::streams::Stream::SeekOrigin::End, -(geomChunksSize + animSetchunksSize + animChunksSize));
                    if (!freader.read(std::span{geomChunksTmp.get(), numGeoms}))
                        return false;
                }

                if (numAnimSets > 0)
                {
                    animSetChunksTmp = std::make_unique<AnimSetChunkInfo[]>(numAnimSets);

                    freader.seek(hr::streams::Stream::SeekOrigin::End, -(animSetchunksSize + animChunksSize));
                    if (!freader.read(std::span{animSetChunksTmp.get(), numAnimSets}))
                        return false;
                }

                if (numAnims > 0)
                {
                    animChunksTmp = std::make_unique<AnimChunkInfo[]>(numAnims);

                    freader.seek(hr::streams::Stream::SeekOrigin::End, -animChunksSize);
                    if (!freader.read(std::span{animChunksTmp.get(), numAnims}))
                        return false;
                }
            }
            ...
            if (geomChunksTmp)
            {
                if (!fwriter.writeObject(geomChunksTmp))
                    return false;
            }
            ...
            if (animSetChunksTmp)
            {
                if (!fwriter.writeObject(animSetChunksTmp))
                    return false;
            }
            ...
            if (animChunksTmp)
            {
                if (!fwriter.writeObject(animChunksTmp))
                    return false;
            }
        ...
        bool fileRemoveData(hr::streams::FileStream& fstreamIn,
                            hr::streams::FileStream& fstreamOut,
                            std::vector<size_t> removeGeomsIds,
                            std::vector<uint32_t> removeAnimSetIds,
                            std::vector<uint32_t> removeAnimIds)
        {
            ...
            // read header and every chunk info
            {
                ...
                if (geomHeader.numGeoms > 0)
                {
                    ...
                    freader.seek(hr::streams::Stream::SeekOrigin::End, -(geomChunksSize + animSetchunksSize + animChunksSize));
                    if (!freader.read(std::span{oldGeomChunks.data(), geomHeader.numGeoms}))
                        return false;
                }
                ...
                if (geomHeader.numAnimSets > 0)
                {
                    ...
                    freader.seek(hr::streams::Stream::SeekOrigin::End, -(animSetchunksSize + animChunksSize));
                    if (!freader.read(std::span{oldAnimSetChunks.data(), geomHeader.numAnimSets}))
                        return false;
                }
                ...
                if (geomHeader.numAnims > 0)
                {
                    ...
                    freader.seek(hr::streams::Stream::SeekOrigin::End, -animChunksSize);
                    if (!freader.read(std::span{oldAnimChunks.data(), geomHeader.numAnims}))
                        return false;
                }
            }

            {
                ...
                removeAnimSetIds.erase(std::remove_if(removeAnimSetIds.begin(), removeAnimSetIds.end(),
                                                      [&oldAnimSetChunks](uint32_t targetId)
                                                      {
                                                          return (std::find_if(oldAnimSetChunks.begin(), oldAnimSetChunks.end(), [targetId](const AnimSetChunkInfo& chunkInfo)
                                                                               { return (chunkInfo.id == targetId); }) == oldAnimSetChunks.end());
                                                      }),
                ...
                removeGeomsIds.erase(std::remove_if(removeGeomsIds.begin(), removeGeomsIds.end(),
                                                    [&oldGeomChunks](uint32_t targetId)
                                                    {
                                                        return (std::find_if(oldGeomChunks.begin(), oldGeomChunks.end(), [targetId](const GeomChunkInfo& chunkInfo)
                                                                             { return (chunkInfo.id == targetId); }) == oldGeomChunks.end());
                                                    }),
                                     removeGeomsIds.end());

                removeAnimIds.erase(std::remove_if(removeAnimIds.begin(), removeAnimIds.end(),
                                                   [&oldAnimChunks](uint32_t targetId)
                                                   {
                                                       return (std::find_if(oldAnimChunks.begin(), oldAnimChunks.end(), [targetId](const AnimChunkInfo& chunkInfo)
                                                                            { return (chunkInfo.id == targetId); }) == oldAnimChunks.end());
                                                   }),
                                    removeAnimIds.end());
            }
            ...
            if (!fwriter.writeObject(geomHeader))
                return false;
            ...
            for (auto& chunkData : newGeomChunks)
            {
                ...
                auto it =
                  std::find_if(oldGeomChunks.begin(), oldGeomChunks.end(), [targetId = chunkData.id](const GeomChunkInfo& chunkInfo) { return (chunkInfo.id == targetId); });
            ...
            for (auto& chunkData : newAnimSetChunks)
            {
                ...
                auto it = std::find_if(oldAnimSetChunks.begin(), oldAnimSetChunks.end(),
                                       [targetId = chunkData.id](const AnimSetChunkInfo& chunkInfo) { return (chunkInfo.id == targetId); });
            ...
            for (auto& chunkData : newAnimChunks)
            {
                ...
                auto it =
                  std::find_if(oldAnimChunks.begin(), oldAnimChunks.end(), [targetId = chunkData.id](const AnimChunkInfo& chunkInfo) { return (chunkInfo.id == targetId); });
            ...
            if (!fwriter.write<GeomChunkInfo>(newGeomChunks))
                return false;

            if (!fwriter.write<AnimSetChunkInfo>(newAnimSetChunks))
                return false;

            if (!fwriter.write<AnimChunkInfo>(newAnimChunks))
                return false;
    ...
    class RendererMaterialProxy: public hr::render::IRenderMaterial
    {
    private:
        size_t mId{0};
    ...
    class RendererTextureSetProxy: public hr::render::IRenderTextureSet
    {
    private:
        size_t mId{0};
    ...
    class RendererObjectProxy: public hr::render::IRenderObject
    {
    public:
        ObjectId objectId{0};
    ...
    private:
        ...
        size_t readVertices(void* const destBuffer, size_t requestedDataSize) const override
        {
            ...
            auto bytesRead = fileStream.read({reinterpret_cast<std::byte*>(destBuffer), requestedDataSize});
    ...
    bool World::geomFileCreate(hr::streams::FileStream& fstream)
    {
        ...
        return hr::streams::StreamWriter(fstream).writeObject(geomHeader);
    }
    ...
    bool World::geomFileAddMesh(hr::streams::FileStream& fstream, size_t geomId, const geom::Mesh<geom::VertexFull, uint32_t>& mesh)
    {
        ...
        size_t numGeoms = 0, numAnimSets = 0, numAnims = 0;
        {
...

            {
                ...
                stream.seek(hr::streams::Stream::SeekOrigin::Begin, 0);
                if (!stream.writeObject(geomHeader))
                    return false;
            }
        }
        ...
        auto success = fileAddData(
          fstream, numGeoms, numAnimSets, numAnims,
          [&mesh = meshShading, &meshBBox, &newGeomChunk](hr::streams::StreamWriter& fwriter)
          {
              newGeomChunk.geomsOffset = fwriter.position();

              if (!fwriter.write<geom::VertexShading>(mesh.vertices()))
                  return false;
              if (!fwriter.write<uint16_t>(mesh.indices()))
                  return false;
          ...
          [&newGeomChunk](hr::streams::StreamWriter& fwriter) { return fwriter.writeObject(newGeomChunk); }, nullptr, nullptr);
    ...
    bool World::geomFileAddMeshAnim(hr::streams::FileStream& fstream, size_t geomId, const hr::geom::MeshAnim& meshAnim, size_t animSetId)
    {
        ...
        auto success = fileAddData(
          fstream, numGeoms, numAnimSets, numAnims,
          [&meshAnim, &newGeomChunk](hr::streams::StreamWriter& fwriter)
          {
              newGeomChunk.geomsOffset = fwriter.position();

              if (!fwriter.write<hr::geom::MeshAnim::TMesh::VertexType>(meshAnim.mesh().vertices()))
                  return false;
              if (!fwriter.write<hr::geom::MeshAnim::TMesh::IndexType>(meshAnim.mesh().indices()))
                  return false;
              ...
              if (!fwriter.write<hr::geom::MeshAnim::VertexJoint>(meshAnim.verticesJoints()))
                  return false;
    ...
    bool World::geomFileAddAnimationSet(hr::streams::FileStream& fstream, size_t animSetId, const hr::geom::SkeletonAnim& skeletonAnim)
    {
        ...
        auto success = fileAddData(
          fstream, numGeoms, numAnimSets, numAnims,
          [&skeletonAnim, &newAnimSetChunk](hr::streams::StreamWriter& fwriter)
          {
          ...
          nullptr, [&newAnimSetChunk](hr::streams::StreamWriter& fwriter) { return fwriter.writeObject(newAnimSetChunk); }, nullptr);
    ...
    bool World::geomFileAddAnimation(hr::streams::FileStream& fstream, size_t animId, size_t animSetId, const hr::geom::SkeletonAnim::Animation& animation)
    {
        ...
        auto success = fileAddData(
          fstream, numGeoms, numAnimSets, numAnims,
          [&animation, &newAnimChunk](hr::streams::StreamWriter& fwriter)
          {
          ...
          nullptr, nullptr, [&newAnimChunk](hr::streams::StreamWriter& fwriter) { return fwriter.writeObject(newAnimChunk); });
    ...
    bool World::geomFileTransformMeshes(hr::streams::FileStream& fstream, std::span<const size_t> geomIds, const std::function<void(geom::Mesh<geom::VertexFull, uint32_t>&)>& cb)
    {
        ...
        for (size_t curGeom = 0; curGeom < geomHeader.numGeoms; ++curGeom)
        {
            ...
            streamBin.read(mesh.vertices());
            streamBin.read(mesh.indices());
...

            {
                ...
                streamWriter.write<geom::VertexShading>(mesh.vertices());
                streamWriter.write<uint16_t>(mesh.indices());
        ...
        // run animations and update vertex data
        {
            ...
            public:
                RendererObjectProxy(size_t objectId, BBox<> bbox, geom::Mesh<geom::VertexShading, uint16_t>& mesh)
                  : mBbox{std::move(bbox)}, mObjectId{objectId}, mMesh{mesh}
                {}
            ...
            struct RendererProxy: public IRenderManager
            {
    ...
    bool World::load(IRenderer& renderer, Area& area, const std::string& scenePath, const std::string& binPath)
    {
        ...
        // read objects and instances
        {
            ...
            // read scene
            {
                ...
                streamScene.stream().cloneAllContent(
                  [&bufferSize, &buffer](size_t requiredSize) mutable
                  {
                      bufferSize = requiredSize;
                      buffer = std::make_shared<unsigned char[]>(requiredSize + 1);

                      buffer.get()[requiredSize] = '\0';
                      return std::span<std::byte>{reinterpret_cast<std::byte*>(buffer.get()), requiredSize + 1};
                  });
                        ...
                        // read geom info and bbox
                        {
...

                            {
                                ...
                                if (object.anim.hasAnim)
                                {
                                    ...
                                    streamBin.read(object.anim.meshAnimated.vertices());
                                    streamBin.read(object.anim.meshAnimated.indices());
                                }
                                else
                                {
                                    // skip base mesh data
                                ...
                                objectRenderer.geom.fstreamIndexOffset =
                                  objectRenderer.geom.fstreamVertexOffset + geom::Mesh<geom::VertexShading, uint16_t>::sizeVertices(objectRenderer.geom.numVertices);
        ...
        // send data to the render
        {
            struct RendererProxy: public IRenderManager
            {
                ...
                RendererProxy(std::map<size_t, RendererMaterialProxy>& rendererMaterials,
                              std::map<size_t, RendererTextureSetProxy>& rendererTextureSets,
                              std::map<size_t, RendererObjectProxy>& renderObjects) noexcept
