...
namespace hr::render
{
    ...
    public:
          ...
          : engine::Runtime::ClassProxy(vm, vmInstance)
        {
            ...
            mScene = std::make_shared<Scene>(stage.mRuntime, stage.mLogger, stage.mFileSystem, stage.mGlCtx, sceneName, sceneFilePath, stage.mRenderData.texColor.width(),
                                             stage.mRenderData.texColor.height());
        }
    ...
    Stage::Stage(engine::Runtime& runtime,
                 engine::Logger::Context& logger,
                 hr::io::FileSystem& fileSystem,
                 hr::gl::objects::Context& glCtx,
                 size_t renderWidth,
                 size_t renderHeight)
      : mRuntime(runtime), mLogger(logger), mFileSystem(fileSystem), mGlCtx(glCtx)
    {
        // render
    ...
    void Stage::processStep()
    {
        ...
        for (auto& scene : mTempScenes)
        {
            ...
            scene->getScene().processStep(scene->getStepDeltaMS(), scene->getStepTotalMS(),
                                          [&](const std::string& msg, const std::string& payload) { msgs.push_back(std::make_pair(msg, payload)); });
