...
namespace hr::render
{
	...
	void RendererDebug::render(RendererMain& rendererMain, const hr::render::World& world, const tools::Camera& hrCamera, const hr::gl::tools::Viewport& hrViewport)
	{
		...
		if (mOptions.drawBBoxes || mOptions.drawNormals)
		{
			...
			hr::gl::glProgramUniformMatrix4fv(mRenderer2D.mShaders.drawNoTex.progVertex.id(), mRenderer2D.mShaders.drawNoTex.progVertex.getUniformLocation("matTrans"), 1, false, matrixTransform.data().data());
