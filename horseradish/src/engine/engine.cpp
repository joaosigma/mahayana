...
namespace hr::engine
{
    ...
    void Engine::runtimeFuncVarList(const std::string& funcName, Runtime::FunctionReturnContext& ctx)
    {
        ...
        mVars.findAllWithKeys([&](const std::string& key, const std::shared_ptr<IVariable>&) { finalList.push_back(key); });
    ...
    void Engine::logSysInfo()
    {
...

        {
            ...
            mLoggerRuntimeCtx->info("   Desktop resolution: {0}x{1}x{2}@{3}", displayWidth.value_or(0), displayHeight.value_or(0), displayColorBits.value_or(0),
                                    displayFrequency.value_or(0));
        }
    ...
    Engine::Engine(std::string_view cmdLine, bool devMode)
      : mDevMode(devMode), mAsyncDispatcher{mAsyncScheduler}
    {
