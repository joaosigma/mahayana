...
namespace HorseRadish
{
	namespace OpenGL
	{
		namespace Tools
		{
			...
			{
			private:
				std::map<std::pair<unsigned int, std::string>, int> mapUniforms;

			public:
				UniformCache();
				~UniformCache();

				void AddUniform(const unsigned int programID, const char * const uniformName, const int uniformPos);
			...
			private:
				...
				std::vector<KerningData> kerningData;
