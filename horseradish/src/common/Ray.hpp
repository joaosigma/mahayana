...
public:
	...
	float DotOrigin(const float &x, const float &y, const float &z) const { return this->origin.Dot(x,y,z); }
	float DotOrigin(const Vector &vec) const { return origin.Dot(vec); }
	float DotDirection(const float &x, const float &y, const float &z) const { return ((this->direction[0] * x) + (this->direction[1] * y) + (this->direction[2] * z)); }
	float DotDirection(const Vector &vec) const { return vec.Dot(this->direction); }
