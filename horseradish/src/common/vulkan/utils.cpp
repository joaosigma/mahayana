...
namespace hr::vulkan
{
	...
	Texture Texture::gen2D(App& app, CommandPool& cmdPool, VkFormat format, const hr::io::FileSystem& fileSystem, std::string_view filePath)
	{
		...
		newTexture.mImage = vulkan::Image::gen2D(app.device(), newTexture.mInfo.width, newTexture.mInfo.height, false, format);
		...
		app.executeOneTimeCommand(cmdPool,
		  [&newTexture, &stagingBuffer](vulkan::CommandBuffer::Recorder& recorder)
		  {
			  ...
			  //actual transfer
			  recorder.copyBufferToImage(newTexture.mImage.native(), stagingBuffer.native(), newTexture.mInfo.width, newTexture.mInfo.height);
		...
		newTexture.mImageView = vulkan::ImageView::gen2D(app.device(), newTexture.mImage.native(), format);
