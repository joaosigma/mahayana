...

            {
                ...
                if (waitDuration && (waitDuration.value().count() > 0))
                {
                    ...
                    else
                    {
                        ...
                        auto upper =
                          std::upper_bound(mWaitQueue.begin(), mWaitQueue.end(), newElement, [](const auto& a, const auto& b) { return (std::get<0>(a) < std::get<0>(b)); });
    ...
    public:
        class Context
        {
            ...
            Context(Scheduler& scheduler, size_t dispatcherId, size_t taskId) noexcept
              : mScheduler{scheduler}, mTaskId{taskId}, mDispatcherId{dispatcherId}
            {}
