...
float BBox::GetRadius(void) const
{
	...
	//tiro o centro da bbox e é indiferente que ponto escolho para o raio (o GetCenter tá a meia distância de ambos)
	GetCenter(center);
	return center.GetDist(minPt);
}
...
float BBox::GetRadiusMinimum(void) const
{
	...
	//dá jeito o centro da bbox
	GetCenter(center);

	//vou calculando e guardando a menor
...
{
	//tiro o centro da bbox e é indefente que ponto escolho para o raio (o GetCenter tá a meia distância de ambos)
	GetCenter(center);
	radius=center.GetDist(minPt);
}
...
void BBox::GetBoundingSphere(BSphere &bsphere) const
{
	...
	//tiro e gravo o centro da esfera
	GetCenter(center);
	bsphere.SetCenter(center);

	//por fim o raio e é indefente que ponto escolho para o raio (o GetCenter tá a meia distância de ambos)
	bsphere.SetRadius(center.GetDist(minPt));
}
...
Vector BBox::GetMainAxis() const
{
	...
	//chegando aqui ou são todos iguais ou iguais 2 a 2
...
BBox::BoxPosition BBox::Classify(const BBox &bbox)
{
	//testo se tá tudo fora
...
bool BSphere::Intersects(const BSphere &bsphere) const
{
	...
	return !(vecDiff.GetSizeSquared() > (radiusSum * radiusSum));
}
...
bool BSphere::Intersects(const Vector &lineStart, const Vector &lineEnd) const
{
	...
	if ( a <= 0.0f )
		return ( s.Dot() < radiusSquared );
	
	if ( a >= r.Dot() )
		return ( e.Dot() < radiusSquared );
	
	r = s + r * ( a / r.Dot() );
	return (r.Dot() < radiusSquared);
}
...
bool BSphere::Intersects(const Ray &ray) const
{
	...
	auto a = rayDir.Dot();
	auto b = rayDir.Dot(p);
	auto c = p.Dot() - (this->radius * this->radius);
	auto d = (b * b) - (c * a);
...
BSphere::POSITION BSphere::ClassifyBSphere(const BSphere &bsphere)
{
	...
	//o centro entre elas
	centerDist = calcDist(bsphere);

	//verifico como estão as esferas em relação a umas às outras
	if ( (centerDist+bsphere.radius) < radius)
		return BSphere::INSIDE;
	if (centerDist-bsphere.radius > radius)
		return BSphere::OUTSIDE;
	return BSphere::INTERSECT;
}
...
BSphere::POSITION BSphere::ClassifyBBox(const BBox &bbox)
{
	...
	//retiro alguns valores
	bbox.GetCorners(pontos);
	numIn=numOut=0;

	//passo pelos primeiro 4 e faço o testes
	for(int i=0; i<4; i++){
		calcDist(pontos[i])<radius ? numIn++ : numOut++;
		}

	//se houver alguns dentro e outros fora, posso logo saber o resultado
	if (numIn!=0 && numOut!=0)
		return BSphere::INTERSECT;

	//tiro as posições dos outro 4
	for(int i=4; i<8; i++){
		calcDist(pontos[i])<radius ? numIn++ : numOut++;
		}

	//agora posso decidir como está a bbox em relação à bsphere
	if (numIn!=0 && numOut!=0)
		return BSphere::INTERSECT;
	if (numIn==0)
		return BSphere::OUTSIDE;
