...
public:
	ConsoleUserHistory()
	{
		...
		//preciso deste espaço para fazer o histórico
		dataMain = new char[this->maxHistorySize];
		if (dataMain==nullptr)
			return;

		//limpo todos os valores
	...
	~ConsoleUserHistory()
	{
		//apago o que tiver anterior
		delete[] dataMain;

		//limpo tudo para evitar desastres
		dataMain = dataNext = dataLast = dataPointer = pointerPush = nullptr;
		numEntradas=0;
	}
	...
	{	
		...
		//não posso exceder o tamanho de cada log
		strLength = strlen(phrase);
		if ((strLength+1) >= this->maxLogSize)
			return false;

		//verifico se dataMain tem espaço, senão limpo
		...
		//posso gravar para dataMain o conteúdo desta frase
		dataLast=dataNext;
		dataNext+=strLength;
...
public:
	...
	void setInput(const char *novaString)
	{
		...
		//vai haver uma mudanca de qualquer maneira
		changeOccured=true;

		//se estiver vazia, é simples
		if (*novaString=='\0')
			{
			cursorPos=0;
			localData.SetEmpty();	
	...
	{
		//se o CONTROL da direita estiver ligado
		if (HIBYTE((WORD)GetKeyState(VK_RCONTROL)))
			{
			//avanco com o cursor (por palavra) para a esquerda
			...
			//avanco com o cursor (por palavra) para a direita
			...
			//não reconheço mais nada com o control
			return;
			}

		//teclas normais, sem o control presionado
		switch(keyVal){
			case VK_LEFT:
							if (cursorPos==0)
								return;
	...
	{
		//se for o caracter #, ou o TAB, ou o '\\' ou o ESC: excuso de seguir
		if (keyVal=='#' || keyVal==9 || keyVal=='\\' || keyVal==27)
			return;

		//se alguém apagou com o CTRL ligado
		if (keyVal==127)
		{
			const char *delStart,*delEnd;
			unsigned int startCountIndex;

			//se a string tá vazia ou o cursor está no inicio
			if (localData.GetSizeChars()==0 || cursorPos==0)
				return;

			//onde deve começar/acabar
			...
			//se não cheguei ao inicio, avanço para não apagar o próprio e o espaço à frente
			if (delStart != localData.GetData())
				delStart += 2;

			//saber em que caracter corresponde o inicio
			startCountIndex = localData.GetCharPosIndex(delStart - localData.GetData());

			//agora só tenho de apagar tudo entre delStart e delEnd inclusivé
			//se não existe nada para mover à frente do delEnd (este tá no fim)
			if (cursorPos >= localData.GetSizeChars())
			{
		...
		if (keyVal==22)
		{
			...
			//tento tirar a primeira string que houver
			...
			{
				//tento tirar o primeiro ficheiro que houver
		...
		if (keyVal==8)
		{
			...
			//se estou no inicio, nao vale a pena
			if (cursorPos==0)
				return;

			//chegando aqui é porque vou apagar um de certeza
			changeOccured=true;
			cursorPos--;

			//se o cursor está no fim
			...
			//basta remover na posicao
			localData.RemoveAt(cursorPos);
			return;
		}

		//chegando aqui, isto de certeza que é um caracter (ou vários depois de passar para unicode)
...
void ConsoleTabConsole::DrawContent(HorseRadish::Render::Renderer2D* const render2D, const HorseRadish::Matrix &transformMatrix)
{
	...
	//calculo isto (dá jeito mais à frente)
	...
	//tenho 1 quadrado a desenhar (por detrás do texto do utilizador)
	...
	//preparo as coisas para desenhar as linhas
	HorseRadish::OpenGL::glLineWidth(2.0f);
	HorseRadish::OpenGL::glEnable(GL_LINE_SMOOTH);

	//tenho de desenhar agora as linhas (em baixo e cima do texto do utilizador)
	...
	//onde fica o texto que o utilizador está a escrever
	userTextY = this->tabContentAreaPos.y + 5.0f;

	//a altura da fonte e onde é suposto escrever o texto
	...
	{
		//retiro a linha e verifico se é para desenhar
		auto curLinha = this->listaTexto + indexLinha;
		if (curLinha->active == false)
			break;

		//mais uma linha e se já desenho fora da janela, posso sair
		textY += textH;
		if (textY >= contentAreaLimits[1])
			break;

		//ignoro linhas vazias (mesmo incluindo só espaços)
		...
		//se tiver uma cor e for a primeira
		...
		//chegando aqui tenho cores de certeza
		
		//preciso disto para controlar a posição e cor a usar no texto
		curX = textX;
		corActual = 0;

		//preciso de um iterator pra percorrer a string
		HorseRadish::String::Iterator it(curLinha->texto);
		for(; (charActual = *it) != '\0'; it++)
		{
			//se a posição actual é uma mudança de cor
			if (it.GetBytePosition() == dataCores[corActual].pos)
			{
				...
				//avanço e se terminei
				corActual++;
				if (corActual >= numCores)
					break;
			}

			//desenho só um caracter
			curX += guiFont->writeChar(curX, textY, charActual);
		}

		//mando escrever o resto da string
	...
	//**************
	//escrevo a string de informação
	...
	//**************
	//verifico se o devo mostrar o cursor
	...
	//**************
	//escrevo o input do utilizador e se tiver cursor, desenho-o
	...
	if (cursorVisivel == true)
	{
		...
		//o caracter onde está o cursor
		cursorChar = unicodeStr.GetUnicodeAt(cursorPos);

		//desenho o cursor e o caracter onde ele está
	...
	//digo à fonte para desenhar o que falta e depois desactivar as coisas que precisa (texturas e ponteiros para GL)
	guiFont->draw();
	guiFont->paintEnd();

	//**************
	//faco o cursor piscar
...
void ConsoleTabConsole::ProcessMSG(const MSG * const msg)
{
	...
	//daqui em diante, só me interressa estes dois tipos, por isso, filtro-os
	if ((msg->message != WM_CHAR) && (msg->message != WM_KEYDOWN))
		return;

	//se é para mexer na consola principal ou no texto
	if (msg->message == WM_KEYDOWN)
	{
		...
		if (msg->wParam == VK_TAB)
		{
			...
			//se não tenho com que trablhar
			if (string[0] == '\0')
				return;

			//elimino os primeiros espaços
			while((string[0] == ' ') && (string[0] != '\0'))
				string++;

			//se depois dos espaços o comando estiver vazio
			if (string[0] == '\0')
			{
			...
			//mando o comando para a consola
			primeiroHit = true;
			dest[0] = '\0';
			...
			//se me mandaram alguma coisa de jeito
			if (dest[0] != '\0')
	...
	//verifico o enter, quero ser eu a tratar dele, se não for vai normalmente
	if (msg->message==WM_CHAR && msg->wParam==13)
	{
		...
		//mando processar o comando
		mainConsole->Process(texto);
		
		//o input actual fica vazio
		userPrompt->setInput("");
		return;
	}

	//manda mensagem para a classe de ler input
	if (msg->message==WM_KEYDOWN)
...
bool ConsoleTabConsole::CriaTextoConsola(HorseRadish::Render::Renderer2D* const renderData)
{
	...
	//obtenho o valor mais pequeno da fonte que agora estou a usar
	textH = renderData->gui.fontConsole->getMaxHeight();

	//conto a porcaria do numero de linhas no maximo
	...
	//tá contado, posso alocar memoria
	this->listaTexto = new LINHA_TEXTO[this->numMaxLinhasTexto];

	//tá criado, mas nenhum está activo
...
void ConsoleTabConsole::ActualizaTextoConsola()
{
	...
	//o lambda que vai fazer a iteracção dos logs
	std::function<bool (const void * const metadata, const int metadataSize, const char * const data, const int dataSize)> iterator = [&](const void * const metadata, const int metadataSize, const char * const data, const int dataSize) -> bool
	{
		...
		for(int indexLinha = 0; indexLinha < this->numMaxLinhasTexto; indexLinha++)
		{
			...
			//gravo os dados e posso sair (pedindo para continuar com o ciclo)
			curLinha->active = true;
			curLinha->texto.Set(HorseRadish::String::UTF8, data);
			curLinha->metadata = (metadataSize <= 0) ? nullptr : metadata;
			return true;
		}

		//chegando aqui não preciso de continuar
		return false;
	};

	//só tenho de iterar pelas entradas
...
void ConsoleTabExtra::DrawContent(HorseRadish::Render::Renderer2D * const render2D, const HorseRadish::Matrix &transformMatrix)
{
	...
	//preciso de escrever o que estou a mostrar
